...
const isAdmin = (cb, uid) => {
    ...
    usersRef().child(uid)
        ...
        .then(() => {
            const index = `${val.isAdmin ? `/company/${val.companyId}/admin/dashboard` : `/company/${val.companyId}/user/profile`}`
            cb(index)
        })
} 
...
export const signupEmployee = ({ companyId, email, password, fullname, cod }, cb) => dispatch => 
createUserWithEmailAndPassword(email,password)
...
const registerEmployee = (companyId, uid, fullname, cod) => {
    companyRef().child(companyId).child(`employees/${uid}`).set({
        cod,
        expenses: 0,
        incomes: 0,
        rentals: 0,

...
export const addCompanyExpense = (companyId, values) =>
  expensesRef(companyId)
    ...
    .then(() =>
      updateAccountingEmployee(companyId, values.uid, values.mount, "expenses")
    )
    .then(() =>
    ...
    .then(() =>
      addEmployeeActivity(companyId, values.uid, {
        type: "EXPENSES",
...
export const addCompanyIncome = (companyId, values) => {
  ...
  incomesRef(companyId)
    ...
    .then(() =>
      updateAccountingEmployee(companyId, values.uid, values.mount, "incomes")
    )
...
export const addPaymentNote = (companyId, values) => {
  ...
  paymentNoteRef(companyId)
    ...
    .then(() =>
      updatePaymentDateByDays(companyId, values.rentalId, values.days)
    );
};
      ...
      paymentNoteRef(companyId)
        ...
        .once("value", snap => {
          addCompanyIncome(companyId, {
            ...
            uid: snap.val().uid,
            description: `Completed in two pays 1-$${snap.val()
              .mount}, 2-$${snap.val().rest}`
          });
...
const updatePaymentDate = (companyId, rentalId, months) =>
  rentsRef(companyId)
    .child(rentalId)
    .once("value", snap =>
      rentsRef(companyId)
        ...
        .update({
          paymentDate:
            snap.val().paymentDate + 1000 * 60 * 60 * 24 * 30 * months
        })
    );

const updatePaymentDateByDays = (companyId, rentalId, days) =>
  rentsRef(companyId)
    .child(rentalId)
    .once("value", snap =>
      rentsRef(companyId)
        .child(rentalId)
        .update({
          paymentDate: snap.val().paymentDate + 1000 * 60 * 60 * 24 * days
        })
    );

const addPaymentNoteToRent = (companyId, rentalId, paymentNote) =>
  rentsRef(companyId)
    .child(rentalId)
    .update({ paymentNote });

const updateAccountingCompany = (companyId, mount, type) =>
  accountingRef(companyId)
    .child(type)
    .transaction(transaction => (transaction += Number(mount)));

const updateAccountingRental = (companyId, rentalId, mount, type) =>
  rentalsRef(companyId)
    .child(rentalId)
    .child(type)
    .transaction(transaction => (transaction += Number(mount)));

const updateAccountingEmployee = (companyId, uid, mount, type) =>
  employeeRef(companyId, uid)
    .child(type)
    .transaction(transaction => (transaction += Number(mount)));

const addEmployeeActivity = (companyId, uid, activity) =>
  employeeActivity(companyId, uid, activity).push(activity);

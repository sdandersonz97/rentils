...
class RentalsOperations extends Component {
  ...
  onScreenChange = (screen, selectedRental) =>
    this.setState({ screen, selectedRental });
  renderOptions = rentalId => {
    ...
    if (employeeRentals[rentalId].available) {
      return [
        {
          name: "EXPENSE",
          onClick: () => this.onScreenChange("ExpensesForm", rentalId)
        }
      ];
      ...
      if (employeeRents[rentalId].paymentNote) {
        return [
          ...
          },
          {
            ...
            onClick: () =>
              history.push(
                `/company/${companyId}/user/paymentnotes/${employeeRents[
                  rentalId
                ].paymentNote}`
              )
          }
        ...
        return [
          ...
          {
            name: "PAYMENT",
            onClick: () => this.onScreenChange("PaymentsForm", rentalId)
          },
          {
            name: "PAYMENT NOTE",
            onClick: () => this.onScreenChange("PaymentNoteForm", rentalId)
          }
        ];
      }
    } else {
  ...
  renderRentalsRows = rentalId => {
    ...
    return (
      <tr key={rentalId}>
        ...
        <td>
          {employeeRents[rentalId]
            ? dayLeft(employeeRents[rentalId].paymentDate)
            : ""}
        </td>
      </tr>
    );
  ...
  renderTableHeader = () => (
    <TableHeader titles={["SELECT", "COD", "ADDRESS", "DISPONIBILITY", ""]} />
  );
  render() {
        ...
        <div className="container-fluid">
          {screen === "RentalList" ? (
            <AssignedRentalListMin
              ...
              render={() =>
                Object.keys(employeeRentals).map(this.renderRentalsRows)}
            />
          ) : screen === "PaymentsForm" ? (
            <IncomesForm
            ...
            <Card>
              <CardBody>{savedMessage}</CardBody>
            </Card>
          )}
        </div>
      </section>
...
export default withRouter(
  connect(mapStateToProps, { fetchAssignments })(RentalsOperations)
);

...
class PaymentNoteForm extends Component {
  ...
  onSubmit = e => {
    ...
    onSaved(
      `Payment note saved: ${employeeRents[selectedRental].tenant} paid ${this
        .state.mount} and the next charge is in ${this.state.days} days`
    );
    this.resetState();
  ...
  render() {
    ...
    return (
      <Card size="9">
        <CardHeader title="Payment Note" category="" color="" />
        <CardBody>
          <form onSubmit={this.onSubmit}>
            <Input label="Tenant" value={tenant} disabled />
            <Input label="Monthly bills" value={price} disabled />
            <Input label="Rest" value={price - mount} disabled />
            <Input
              label="Mount"
              type="number"
              value={mount}

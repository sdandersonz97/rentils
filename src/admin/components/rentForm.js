...
class RentForm extends Component {
  ...
  onSelectEmployee = ({ target }) =>
    this.setState({ selectedEmployee: target.value });
  renderEmployeeRows = employeeId => {
    ...
    return (
      <tr key={employeeId}>
        ...
        <td>{employees[employeeId].cod}</td>
        <td>{employees[employeeId].fullname}</td>
        <td>{employees[employeeId].rentals}</td>
      </tr>
    );
  };
            ...
            {screen === "RentForm"
              ...
              : [
                  <Table key="employeeTable">
                    <TableHeader
                      titles={["SELECT", "COD", "FULL NAME", "RENTALS"]}
                    />
                    <TableBody>

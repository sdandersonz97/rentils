...
class EmployeesList extends Component {
  ...
  renderRows = uid => {
    ...
    return (
      <tr key={uid}>
        <td>{employeesList[uid].cod}</td>
        <td>
          <Link to={`/company/${companyId}/admin/employees/employee/${uid}`}>
  ...
  filterEmployeesList = () => {
    ...
    switch (filter) {
      ...
      case "incomes":
        return employeesListKeys.sort(
          (a, b) => employeesList[b].incomes - employeesList[a].incomes
        );
      case "expenses":
        return employeesListKeys.sort(
          (a, b) => employeesList[b].expenses - employeesList[a].expenses
        );
      case "rentals":
        return employeesListKeys.sort(
          (a, b) => employeesList[b].rentals - employeesList[a].rentals
        );
      case "rents":
        return employeesListKeys.sort(
          (a, b) => employeesList[b].rents - employeesList[a].rents
        );
    }
  };
  render() {
...
export default withRouter(
  connect(mapStateToProps, { fetchCompanyEmployees })(EmployeesList)
);
